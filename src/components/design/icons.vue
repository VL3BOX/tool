<template>
    <div class="m-icons">
        <h1 class="m-icons-title">{{ $t('剑三图标库') }}</h1>
        <div class="m-icons-box">
            <el-radio-group v-model="mode" class="m-icons-radio">
                <el-radio-button label="search">{{ $t('探索模式') }}</el-radio-button>
                <el-radio-button label="view">{{ $t('浏览模式') }}</el-radio-button>
            </el-radio-group>
            <IconsSearch v-if="mode === 'search'" />
            <IconsView v-if="mode === 'view'" />
            <!-- <el-tabs v-model="activeTabName" type="card">
                <el-tab-pane :label="$t('图标库')" name="list">
                </el-tab-pane>
                <el-tab-pane :label="$t('收藏图标')" name="favicon">
                    <IconsFav />
                </el-tab-pane>
            </el-tabs> -->
        </div>
